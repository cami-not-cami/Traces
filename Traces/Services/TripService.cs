...
namespace Traces.Services
{
    public class TripService : ITripService
    {
        ...
        public async Task<GooglePlaceResponse?> GetGooglePlaceDetailsAsync(string placeId)
        {
            ...
            if (string.IsNullOrEmpty(json) || json.StartsWith("Error calling Google Places API") || json.StartsWith("Place ID cannot be null"))
            {
                return null;
            }
            return JsonSerializer.Deserialize<GooglePlaceResponse>(json, new JsonSerializerOptions { PropertyNameCaseInsensitive = true });
        }
        ...
        public async Task LinkUserToTripAsync(int tripId, string? userId = null, string? userEmail = null, string tripMemberEmail = "")
        {
        ...
        public async Task MigrateSessionTripsAsync(List<int> sessionTrips, string userId, string? userEmail)
        {
        ...
        public async Task<List<Trip>> GetSessionTripsAsync(List<int> tripIds)
        {
            return await _context.Trips
                .Where(t => tripIds.Contains(t.TrIdPk))
                .ToListAsync();
        }

        public async Task<int> CreateTripAsync(CreateTripViewModel model, string? userId, string? userEmail)
        {
            var trip = new Trip
            {
            ...
            string? userEmail)
        {
            ...
            decimal? lat = null;
            if (decimal.TryParse(latitude, NumberStyles.Any, CultureInfo.InvariantCulture, out var parsedLat))
            {
                lat = parsedLat;
            }

            decimal? lng = null;
            if (decimal.TryParse(longitude, NumberStyles.Any, CultureInfo.InvariantCulture, out var parsedLng))
            {
                lng = parsedLng;
            }
        ...
        public async Task ReorderTimelineItemsAsync(int tripDayId, List<ReorderTimelineItemDto> items)
        {
            var affectedDayIds = new HashSet<int> { tripDayId };
        ...
        public async Task DeleteTripAsync(int tripId)
        {
            ...
            var activityIds = await _context.TripActivities
                .Where(a => dayIds.Contains(a.TripDayFk))
                .Select(a => a.TrAcIdPk)
                .ToListAsync();
        ...
        public async Task RemoveTripMemberAsync(int tripId, int memberId)
        {
            var member = await _context.TripMembers
                .FirstOrDefaultAsync(tm => tm.TripFk == tripId && tm.IdFk == memberId);
            if (member != null)
            {
        ...
        public async Task UpdateRouteTravelModeAsync(int fromActivityId, int toActivityId, string travelMode)
        {
            var route = await _context.RouteToNexts
                .FirstOrDefaultAsync(r => r.FromActivityFk == fromActivityId && r.ToActivityFk == toActivityId);
            ...
            var fromActivity = await _context.TripActivities
                .Include(a => a.PlaceFkNavigation)
                .FirstOrDefaultAsync(a => a.TrAcIdPk == fromActivityId);

            var toActivity = await _context.TripActivities
                .Include(a => a.PlaceFkNavigation)
            ...
            if (!string.IsNullOrEmpty(originalPlaceId) && !string.IsNullOrEmpty(destinationPlaceId))
            {
                var routeInfo = await _googleMapsServices.GetDirectionsBetweenRoutes(originalPlaceId, destinationPlaceId, travelMode);
                if (routeInfo != null)
                {
